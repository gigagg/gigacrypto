...
export class Keychain {
  ...
  private async doImport(k: LockedKeychain) {
    ...
    console.log(toBase64(this.masterKey));
...
class PkcsReader {
  ...
  private readMPInt(nm: string) {
    ...
    this.offset = offset;
    let slice = this.buf.slice(this.offset, this.offset + len);
    this.offset += len;
...
class PkcsWriter {
  ...
  private startSequence(tag: number = 16 + 32) {
    ...
    console.log('Start seq', this.data.length);
  ...
  private endSequence() {
    ...
    if (len <= 0x7f) {
      ...
      console.log('End seq with 1 byte', seq, len);
    } else if (len <= 0xff) {
      this.data.splice(seq, 0, 0x81, len);
      console.log('End seq with 2 byte', seq, len);
    } else if (len <= 0xffff) {
      console.log('End seq with 3 byte', seq, len);
      this.data.splice(seq, 0, 0x82, (len >> 8) & 0xff, len & 0xff);
    } else if (len <= 0xffffff) {
      console.log('End seq with 4 byte', seq, len);
  ...
  writeRsaPrivateKey(key: RsaPrivate): Uint8Array {
    ...
    this.writeBuffer(key.n, 2);
    this.writeBuffer(key.e, 2);
    this.writeBuffer(key.d, 2);
    this.writeBuffer(key.p, 2);
    this.writeBuffer(key.q, 2);
    // if (!key.part.dmodp || !key.part.dmodq)
    // 	utils.addRSAMissing(key);
    this.writeBuffer(key.dmodp, 2);
    this.writeBuffer(key.dmodq, 2);
    this.writeBuffer(key.iqmp, 2);
